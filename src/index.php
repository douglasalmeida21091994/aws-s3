...
<div class="container">
    <div class="upload-box">
        <div class="header">
            ...
            <p>Armazenamento Seguro na Nuvem</p>
            <div class="aws-badges">
                <span class="badge"><i class="fab fa-aws"></i> S3</span>
                <span class="badge"><i class="fas fa-database"></i> RDS</span>
            </div>
        </div>
        ...
        <form id="uploadForm">
            ...
            <div class="form-group">
                ...
                <div class="file-input-wrapper" id="fileWrapper">
                    <input type="file" id="file_input" accept=".pdf,.doc,.docx">
                    <div class="file-icon">
...
$(document).ready(function() {
    // Cores do SweetAlert condizentes com o tema dark
    const swalConfig = {
        background: '#1e293b',
        color: '#e2e8f0',
    ...
    // CORREÇÃO: Manipulação do clique para evitar recursão
    $('#fileWrapper').on('click', function(e) {
        // Se o clique veio diretamente do input, não fazemos nada (evita o loop)
        if (e.target.id === 'file_input') return;
        
        // Caso contrário, força o clique no input
        $('#file_input').click();
    });

    // Impede que o clique no input "suba" para o wrapper
    $('#file_input').on('click', function(e) {
        e.stopPropagation();
    });

    // O restante do código (change, dragover, drop) permanece igual
    ...
    $('#uploadForm').on('submit', function(e) {
        ...
        formData.append('arquivo', $('#file_input')[0].files[0]);
        ...
        Swal.fire({
            ...swalConfig,
            title: 'Processando...',
            text: 'Enviando dados para AWS S3 e RDS',
        ...
        $.ajax({
            ...
            success: function(resp) {
                if (resp.status == 'ok') {
                    Swal.fire({
                        ...
                        icon: 'success',
                        title: 'Sucesso!',
                        text: 'Arquivo armazenado com sucesso.'
            ...
            complete: () => {
                $('#btnUpload').prop('disabled', false).html('<i class="fas fa-upload"></i> Fazer Upload');
            }
        });
    });
});
